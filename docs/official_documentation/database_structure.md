...
| `deleted_at` | TIMESTAMP | INDEX (partial: WHERE deleted_at IS NULL) | Soft-delete marker (NULL = live) |

**Sample Data:**
```sql
INSERT INTO item_type (item_type_name) VALUES 
('Raw Material'), ('Finished Good'), ('Component'), ('Packaging'), ('Consumable'), ('Bundle');
```
...
erDiagram
    ...
    inventory_location {
        ...
        varchar255 display_code "auto-generated by trigger"
        boolean is_active "DEFAULT TRUE INDEX"
        int sort_order "pick-path ordering"
        timestamp created_at "DEFAULT NOW()"
...
| `is_active` | BOOLEAN | DEFAULT TRUE, INDEX | Active status |
| `deleted_at` | TIMESTAMP | INDEX, nullable | Soft-delete marker (NULL = live); cascade soft-deletes all child InventoryLocation rows |
| `location_count` | INTEGER | computed | Virtual field: count of non-deleted InventoryLocation rows for this warehouse (populated via LEFT JOIN subquery in API responses) |
| `created_at` | TIMESTAMP | DEFAULT NOW() | Creation timestamp |
| `updated_at` | TIMESTAMP | DEFAULT NOW() | Last update timestamp |
...
erDiagram
    platform {
        ...
        boolean is_online "DEFAULT TRUE — online vs offline store"
        timestamp created_at "DEFAULT NOW()"
    }
...
| `is_online` | BOOLEAN | DEFAULT TRUE | True = online marketplace, False = offline/physical store |
| `created_at` | TIMESTAMP | DEFAULT NOW() | Creation timestamp |
...
| `trg_items_history_on_insert` | items | AFTER INSERT | Auto-insert audit trail record into items_history |
| `trg_items_history_on_update` | items | AFTER UPDATE | Auto-insert audit trail with old/new values; operation = UPDATE / SOFT_DELETE / RESTORE |
...
**5. `insert_items_history_on_create()`** - Auto-inserts an audit trail record into `items_history` with `operation = 'INSERT'` whenever a new item (including bundles) is created. Captures the full initial state (`item_name`, `master_sku`, `sku_name`, `item_type_id`, `category_id`, `brand_id`, `uom_id`, `is_active`, `has_variation`, `parent_id`, `image_url`) as JSONB `snapshot_data`.

**6. `insert_items_history_on_update()`** - Auto-inserts an audit trail record into `items_history` whenever an item is updated. Captures both old and new state in JSONB `snapshot_data`:
- **Operation detection:** `SOFT_DELETE` (deleted_at set), `RESTORE` (deleted_at cleared), `UPDATE` (all other changes)
- **Snapshot format:** New values at top level, old values nested under `previous_values` key
- **User tracking:** Uses PostgreSQL session variable `app.current_user_id` (set by FastAPI endpoints via `SET LOCAL`), falls back to `deleted_by` for soft-delete/restore operations
...
Identifies bundle listings from `listing_component` using two criteria: (a) listings with more than one unique `item_id`, OR (b) listings with a single `item_id` where `quantity > 1`.

**Key Columns:** listing_id, platform_sku, platform_name, seller_name, component_count, total_bundle_quantity, listing_active

**Source Tables:** listing_component → platform_sku → platform → seller
