...
import { importBundles } from '../../api/base/items';
...
const CSV_TEMPLATE =
  'bundle_name,bundle_sku,component_sku,component_qty,sku_name,description,category,brand,uom,is_active\n' +
  'Summer Essentials Pack,BUNDLE-SUMMER-001,TSHIRT-001,2,Summer Pack,A summer bundle,Apparel,My Brand,Each,true\n' +
  'Summer Essentials Pack,BUNDLE-SUMMER-001,SHORTS-001,1,,,,,,\n' +
  'Winter Warmth Bundle,BUNDLE-WINTER-001,JACKET-001,1,Winter Bundle,A winter bundle,Apparel,My Brand,Each,true\n' +
  'Winter Warmth Bundle,BUNDLE-WINTER-001,SCARF-001,3,,,,,,\n';
...
function downloadTemplate() {
  ...
  a.href = url;
  a.download = 'bundles_upload_template.csv';
  a.click();
...
export default function BundlesMassUploadPage({ hideHeader = false }: { hideHeader?: boolean } = {}) {
  ...
  const handleConfirmUpload = async () => {
    ...
    setSubmitError(null);
    try {
      const res = await importBundles(file);
      setResult(res);
      setFile(null);
      setPreviewHeaders([]);
      setPreviewRows([]);
    ...
    <div className="max-w-4xl mx-auto">
      ...
      {!hideHeader && (
        <div className="flex items-center gap-3 mb-6">
          <h1 className="text-2xl font-semibold text-text-primary">Mass Upload Bundles</h1>
        </div>
      )}
      ...
      <div className="bg-surface rounded-card shadow-card p-5 mb-5">
        ...
        <ol className="text-sm text-text-secondary space-y-1.5 list-decimal list-inside mb-4">
          ...
          <li>
            Each row represents one <span className="font-medium text-text-primary">component</span> of a bundle.
            Rows with the same{' '}
            <span className="font-medium text-text-primary">bundle_sku</span>{' '}
            are grouped into a single bundle.
          </li>
          <li>
            <span className="font-medium text-text-primary">bundle_name</span>,{' '}
            <span className="font-medium text-text-primary">bundle_sku</span>,{' '}
            <span className="font-medium text-text-primary">component_sku</span>, and{' '}
            <span className="font-medium text-text-primary">component_qty</span>{' '}
            are required for every row.
          </li>
          <li>
            Optional metadata (<em>category, brand, uom, description, is_active</em>) is taken
            from the <strong>first row</strong> of each bundle group. Later rows for the same bundle
            only need the component columns.
          </li>
          <li>
            Component SKUs must match existing items in your catalog. Add items first if needed.
          </li>
          ...
          <div className="flex flex-wrap gap-3 mb-5">
            {result.success_rows > 0 && (
              <div className="flex items-center gap-2 px-4 py-3 bg-success-bg text-success-text rounded-default text-sm font-medium">
                ...
                {result.success_rows} bundle{result.success_rows !== 1 ? 's' : ''} imported successfully
              </div>
            )}
            {result.error_rows > 0 && (
              <div className="flex items-center gap-2 px-4 py-3 bg-error-bg text-error-text rounded-default text-sm font-medium">
                <ErrorOutlineIcon fontSize="small" />
                {result.error_rows} bundle{result.error_rows !== 1 ? 's' : ''} had errors
              </div>
            )}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-background border-b border-divider">
                      ...
                      <th className="px-4 py-2.5 text-left text-xs font-semibold text-text-secondary uppercase tracking-wider w-48">Bundle SKU</th>
