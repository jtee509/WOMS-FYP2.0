-- =============================================================================
-- WOMS PostgreSQL Views
-- 
-- This file contains reporting views for the WOMS database.
-- Run this after Alembic migrations and triggers have been created.
--
-- Views:
-- 1. v_inventory_status - Real-time inventory status with alerts
-- 2. v_order_fulfillment - Order fulfillment summary
-- 3. v_seller_warehouse_routing - Seller warehouse assignments
-- 4. v_platform_import_status - Raw import processing status
-- 5. v_active_inventory_alerts - Active alerts dashboard
-- =============================================================================
...
SELECT 
    ...
    CASE 
        ...
        WHEN il.max_stock IS NOT NULL AND il.quantity_available >= il.max_stock THEN 'OVERSTOCK'
        ELSE 'OK'
    END AS stock_status,
...
-- =============================================================================
-- 2. ORDER FULFILLMENT VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_order_fulfillment;
...
'Order fulfillment summary with aggregated line items, amounts, and status';

-- =============================================================================
-- 3. SELLER WAREHOUSE ROUTING VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_seller_warehouse_routing;
...
COMMENT ON VIEW v_seller_warehouse_routing IS 
'Seller to warehouse fulfillment routing with priority and statistics';

-- =============================================================================
-- 4. PLATFORM IMPORT STATUS VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_platform_import_status;
...
COMMENT ON VIEW v_platform_import_status IS 
'Platform raw import processing status with normalization tracking';

-- =============================================================================
-- 5. ACTIVE INVENTORY ALERTS VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_active_inventory_alerts;
...
COMMENT ON VIEW v_active_inventory_alerts IS 
'Active inventory alerts dashboard with priority ranking';

-- =============================================================================
-- 6. WAREHOUSE CAPACITY SUMMARY VIEW
-- =============================================================================
...
SELECT 
    ...
    COUNT(DISTINCT o.order_id) FILTER (WHERE o.order_status = 'pending') AS pending_orders,
...
COMMENT ON VIEW v_warehouse_summary IS 
'Warehouse summary with inventory, alerts, and resource statistics';

-- =============================================================================
-- 7. ORDER LINE ITEM DETAILS VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_order_line_items;
...
COMMENT ON VIEW v_order_line_items IS 
'Detailed order line items with SKU translation and fulfillment status';

-- =============================================================================
-- 8. ORDER RETURNS VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_order_returns;
...
COMMENT ON VIEW v_order_returns IS 
'Order returns with status, inspection, and restock tracking';

-- =============================================================================
-- 9. ORDER EXCHANGES VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_order_exchanges;
...
COMMENT ON VIEW v_order_exchanges IS 
'Order exchanges with original/new item details and value adjustments';

-- =============================================================================
-- 10. ORDER MODIFICATIONS VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_order_modifications;
...
COMMENT ON VIEW v_order_modifications IS 
'Order modification audit trail with old/new values and related operations';

-- =============================================================================
-- 11. ORDER PRICE ADJUSTMENTS VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_order_price_adjustments;
...
COMMENT ON VIEW v_order_price_adjustments IS 
'Order price adjustments with related exchanges and modifications';

-- =============================================================================
-- 12. ORDER OPERATIONS SUMMARY VIEW
-- =============================================================================

DROP VIEW IF EXISTS v_order_operations_summary;
...
SELECT 
    ...
    COUNT(DISTINCT r.id) FILTER (WHERE r.return_status NOT IN ('completed', 'cancelled', 'rejected')) AS active_returns,
    
    -- Exchange counts
    COUNT(DISTINCT e.id) AS total_exchanges,
    COUNT(DISTINCT e.id) FILTER (WHERE e.exchange_status NOT IN ('completed', 'cancelled')) AS active_exchanges,
    ...
    SUM(CASE WHEN pa.status = 'applied' THEN pa.adjustment_amount ELSE 0 END) AS total_adjustment_applied,
...
COMMENT ON VIEW v_order_operations_summary IS 
...
-- Uncomment if you need faster reporting queries
-- Remember to refresh periodically: REFRESH MATERIALIZED VIEW mv_daily_order_summary;
...
SELECT 
    ...
    SUM(od.paid_amount) AS total_revenue,
    SUM(od.discount) AS total_discounts,
    SUM(od.shipping_fee) AS total_shipping,
...
LEFT JOIN warehouse w ON o.assigned_warehouse_id = w.id
JOIN order_details od ON o.order_id = od.order_id
GROUP BY DATE(o.order_date), p.platform_name, s.store_name, w.warehouse_name;

CREATE UNIQUE INDEX idx_mv_daily_summary 
    ON mv_daily_order_summary (order_date, platform_name, store_name, warehouse_name);
*/
...
-- SELECT * FROM v_inventory_status WHERE stock_status != 'OK' LIMIT 10;

-- Test order fulfillment view
-- SELECT * FROM v_order_fulfillment WHERE overall_fulfillment_status = 'PENDING' LIMIT 10;
