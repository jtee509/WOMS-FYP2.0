...
class InventoryLocation(SQLModel, table=True):
    ...
    def full_location_code(self) -> str:
        """Generate full location code from hierarchy."""
...
class InventoryLevel(SQLModel, table=True):
    ...
    def stock_status(self) -> str:
        """Determine current stock status based on thresholds."""
        if self.quantity_available == 0:
            return "OUT_OF_STOCK"
        if self.safety_stock and self.quantity_available <= self.safety_stock:
            return "CRITICAL"
        if self.reorder_point and self.quantity_available <= self.reorder_point:
            return "LOW"
        if self.max_stock and self.quantity_available >= self.max_stock:
            return "OVERSTOCK"
        return "OK"
